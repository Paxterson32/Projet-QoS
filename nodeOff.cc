...
// Fonction permettant la desactivation du noeud
void TurnOffNode(Ptr<Node> node) {
    ...
    // Alternativement, pour désactiver un seul dispositif réseau :
    // Ptr<NetDevice> device = node->GetDevice(0); // Index 0 pour le premier dispositif
    // device->SetAttribute("IsUp", BooleanValue(false));

    // Mise à jour des tables de routage après la panne du noeud
...
int main(int argc, char *argv[])
{
   ...
   // Creation d'une connexion p2p entre le N8 et le LAN 2 : 
    ...
    pointToPoint.SetDeviceAttribute("DataRate", StringValue("11Kbps")); //Diminution du dataRate et auggmentation du Delai
    ...
    Simulator::Schedule(Seconds(2.3), &TurnOffNode, p2pNodes.Get(1));
