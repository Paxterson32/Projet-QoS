...
int main(int argc, char *argv[])
{
    ...
    std::string dataRate("10kbps"); // application layer data rate
    CommandLine cmd;
    //cmd.AddValue("verbose", "Tell echo applications to log if true", verbose);
    cmd.AddValue ("rate", "Rate for the rate File ", rate);
    //cmd.AddValue ("stopTime", "Date de fin de la simulation ",stopTime);

    cmd.Parse(argc, argv);

    NS_LOG_UNCOND("Le rate est : " << rate);
   ...
   // Creation d'une connexion p2p entre le N8 et le LAN 2 : 
    ...
    pointToPoint.SetDeviceAttribute("DataRate", StringValue(dataRate)); //Diminution du dataRate
    ...
    echoClient2.SetAttribute("MaxPackets", UintegerValue(packets));
    ...
    // Ouverture du fichier pour contenir les resultats : 
   //std::ofstream file("output.txt",std::ios_base::app);
    std::ofstream lostFile("lost.txt",std::ios_base::app);
    std::ofstream debitFile("debit.txt",std::ios_base::app);
    std::ofstream averageDelay("average.txt",std::ios_base::app);
    ...
    for (std::map<FlowId, FlowMonitor::FlowStats>::const_iterator i = stats.begin (); i != stats.end(); ++i)
    {
        ...
        // Redirecting the output inside a file : 
        // file << "Flow " << i->first << " (" << t.sourceAddress << " -> " << t.destinationAddress << ")\n";
        //file << i->second.txBytes << " " << i->second.rxBytes << " " << i->second.txPackets << " " << i->second.lostPackets << " " << i->second.rxPackets << " " << i->second.timesForwarded << " " << i->second.delaySum.GetSeconds() << " " << i->second.rxBytes* 8.0 / (i->second.timeLastRxPacket.GetSeconds() - i->second.timeFirstTxPacket.GetSeconds())/1024 << "\n";
        lostFile << rate << " " << i->second.lostPackets << "\n";
        debitFile << rate << " " << i->second.rxBytes* 8.0 / (i->second.timeLastRxPacket.GetSeconds() - i->second.timeFirstTxPacket.GetSeconds())/1024 << "\n";
        averageDelay << rate << " " << i->second.delaySum.GetSeconds() << "\n";
